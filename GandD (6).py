...
class Blur2d(nn.Module):
    def __init__(self, channels, kernel=(1, 2, 1), pad_mode="reflect"):
        super().__init__()
        ...
        self.pad = (k.shape[-1] - 1) // 2
        self.pad_mode = pad_mode

    def forward(self, x):
        if self.pad > 0:
            x = F.pad(x, (self.pad, self.pad, self.pad, self.pad), mode=self.pad_mode)
        return F.conv2d(x, self.kernel, stride=1, padding=0, groups=self.channels)
...
class UpBlock(nn.Module):
    def __init__(
        ...
        self.up = nn.Sequential(
            conv2d(in_ch, out_ch * 4, 1, 1, 0, bias=False),
            nn.PixelShuffle(2),
            batchNorm2d(out_ch),
            Blur2d(out_ch, kernel=(1, 2, 1)),
        )
