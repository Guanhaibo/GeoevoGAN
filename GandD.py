...
class GrassmannSeed(nn.Module):
    def __init__(self, z_dim, K, rank, field_res):
        super().__init__()
        self.K = K
        self.rank = rank
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=256):
        ...
        self.im_size = im_size
         ...
         # GrassmannSeed: 同时生成初始特征 g0 (4x4) 和条件 q
        self.seed = GrassmannSeed(z_dim=nz, K=cfg.K, rank=cfg.rank, field_res=4)
        self.g0_to_4 = nn.Sequential(
            conv2d(cfg.K, nfc[4], 1, 1, 0, bias=False),
            #batchNorm2d(nfc[4]),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.feat_8 = UpBlock(
...
class Discriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=256):
        ...
        # 仅增强 logits 的 big 分支，不动共享 trunk / recon 分支
        self.adv_refine_big = nn.Sequential(
            Star(
                dim=nfc[16],
                mlp_ratio=1.5,
                gamma_init=0.0,
    ...
    def _score(self, feat_last, feat_small):
        feat_last_adv = self.adv_refine_big(feat_last)

        rf_big = self.rf_big(feat_last_adv).reshape(feat_last.size(0), -1).squeeze(1)
